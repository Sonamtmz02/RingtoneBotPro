...
async def search_ringtone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for idx, video in enumerate(ringtone_videos[:3]):
            ...
            status_msg = await update.message.reply_text(f"Downloading {idx+1}/3...")
                ...
                try:
                    with open(file_path, 'rb') as audio_file:
                        await update.message.reply_audio(
                            ...
                            filename=f"ringtone_{idx+1}.mp3"
                        )
                    os.remove(file_path)
                    sent_count += 1
                    ...
                    await status_msg.edit_text(f"Failed to send: {video_title[:50]}")
            else:
                await status_msg.edit_text(f"Download failed for ringtone {idx+1}")
        
        if sent_count > 0:
            await search_msg.delete()
        else:
            await search_msg.edit_text("All downloads failed. Please try again.")
